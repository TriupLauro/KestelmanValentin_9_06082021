...
export default class {
  constructor({ document, onNavigate, firestore, bills, localStorage }) {
    ...
    this.arrowState = ['unused','closed','closed','closed']
  ...
  handleShowTickets(e, bills, index) {
    ...
    if (this.arrowState[index] === 'closed') {
      this.filtered = filteredBills(bills, getStatus(this.index))
      $(`#arrow-icon${this.index}`).css({ transform: 'rotate(0deg)'})
      $(`#status-bills-container${this.index}`)
        .html(cards(this.filtered))
      this.arrowState[index] = 'open'
    } else {
      this.filtered = []
